...
pub fn stage_selector(props: &Props) -> Html {
    ...
    let save_stage_onclick = Callback::from(move |_: i32| { 
        ...
        let territory_id = cloned_state.territory_id;
        let to_stage_id = cloned_state.stage_id;
        spawn_local(async move {
            let uri_string: String = format!("{path}?territoryId={territory_id}&stageId={to_stage_id}&assignee={assignee}", 
                ...
                assignee = cloned_state.signed_out_to.clone().unwrap_or_default(),
            );
    ...
    let selected_stage_id = props.value;
